...
def bin_goal_difference(diff_series: pd.Series) -> pd.Series:
    ...
    conditions = [
        diff_series >= 3, diff_series == 2, diff_series == 1, diff_series == 0,
        diff_series == -1, diff_series == -2, diff_series <= -3,
    ]
...
def create_final_feature_matrix(input_filename: str, output_filename: str):
    """
    Loads football data and creates a final one-hot encoded feature matrix
    with all requested categories.
    """
    print(f"Starting final feature matrix generation for '{input_filename}'...")
    ...
    # --- 4. Generate All Category Columns ---
    print("Generating all specified one-hot encoded columns...")
    ...
    static_cats = {
        'fav_win': (df['ft_diff'] > 0), 'draw': (df['ft_diff'] == 0), 'fav_loss': (df['ft_diff'] < 0),
        'no_draw': (df['ft_diff'] != 0), # <-- ADDED
        'fav_double_chance': (df['ft_diff'] >= 0), 'opp_double_chance': (df['ft_diff'] <= 0),
        'fav_win_by_2+': (df['ft_diff'] >= 2),
        'fav_win_to_nil': (df['ft_diff'] > 0) & (df['ft_opp'] == 0),
        'fav_failed_to_score': (df['ft_fav'] == 0), # <-- ADDED
        'opp_failed_to_score': (df['ft_opp'] == 0), # <-- ADDED
        ...
        'fav_comeback_draw': (df['ht_diff'] < 0) & (df['ft_diff'] == 0), # <-- ADDED
        'fav_threw_lead_draw': (df['ht_diff'] > 0) & (df['ft_diff'] == 0), # <-- ADDED
    ...
    # C. Unbinned Dynamic Categories (HT/FT)
...
if __name__ == "__main__":
    ...
    OUTPUT_FILE = 'fikstur_feature_matrix_final.csv'
    
    create_final_feature_matrix(INPUT_FILE, OUTPUT_FILE)